...
mping_protocol = Proto("mping", "Multicast Ping Protocol")
...
f_host_family_lookup = {
    [2] = "AF_INET",
    [10] = "AF_INET6"
}
print(_VERSION) 
f_version = ProtoField.stringz("mping.version", "Version", base.ASCII)
f_type = ProtoField.char("mping.type", "Type", base.NONE, f_type_lookup)
f_ttl = ProtoField.uint8("mping.ttl", "TTL", base.DEC)

f_source_host_family = ProtoField.uint16("mping.src.family", "Source Address Family", base.DEC, f_host_family_lookup)
f_source_host_ipv4 = ProtoField.ipv4("mping.src.ipv4", "Source IPv4")
f_source_host_ipv6 = ProtoField.ipv6("mping.src.ipv6", "Source IPv6")
f_source_host_port = ProtoField.uint16("mping.src.port", "Source Port", base.DEC)

f_destination_host_family = ProtoField.uint16("mping.dst.type", "Destination Address Family", base.DEC, f_host_family_lookup)
f_destination_host_ipv4 = ProtoField.ipv4("mping.dst.ipv4", "Source IPv4")
f_destination_host_ipv6 = ProtoField.ipv6("mping.dst.ipv6", "Source IPv6")
f_destination_host_port = ProtoField.uint16("mping.dst.port", "Destination Port", base.DEC)

f_sequence_number = ProtoField.uint16("mping.seq_no", "Sequence number", base.DEC)
f_pid = ProtoField.uint16("mping.pid", "PID", base.DEC)

f_seconds = ProtoField.uint64("mping.timestamp.seconds", "Seconds", base.DEC)
f_microseconds = ProtoField.uint64("mping.timestamp.microseconds", "Microseconds", base.DEC)

e_depracated_version = ProtoExpert.new("mping.depracated_version", "Depracated version", expert.group.DEPRECATED, expert.severity.NOTE)
e_invalid_type = ProtoExpert.new("mping.invalid_type", "Invalid Type", expert.group.MALFORMED, expert.severity.ERROR)

e_invalid_source_host_family = ProtoExpert.new("mping.src.invalid_family", "Invalid Source Address Family", expert.group.MALFORMED, expert.severity.ERROR)
f_mismatch_source_host_address = ProtoExpert.new("mping.src.mismatch_addr", "Source address does not match the one in the upper header.", expert.group.PROTOCOL, expert.severity.WARN)
f_mismatch_source_host_port = ProtoExpert.new("mping.src.mismatch_port", "Source Port does not match the one in the upper header.", expert.group.PROTOCOL, expert.severity.WARN)

e_invalid_destination_host_family = ProtoExpert.new("mping.dst.invalid_family", "Invalid Destination Address Family", expert.group.MALFORMED, expert.severity.ERROR)
f_mismatch_destination_host_address = ProtoExpert.new("mping.dst.mismatch_addr", "Destination address does not match the one in the upper header.", expert.group.PROTOCOL, expert.severity.WARN)
f_mismatch_destination_host_port = ProtoExpert.new("mping.dst.mismatch_port", "Destination Port does not match the one in the upper header.", expert.group.PROTOCOL, expert.severity.WARN)

e_timestamp = ProtoExpert.new("mping.timestamp.timestamp", "corresponds to absolute time: [to fill out]", expert.group.COMMENTS_GROUP, expert.severity.COMMENT)
...
function mping_protocol.dissector(buffer, pinfo, tree)
    ...
    if (seconds < max_uint64 and microseconds < max_uint64) then -- is :tonumber() safe to use?
        local timestamp = NSTime.new(seconds:tonumber(), microseconds:tonumber() * 1000)
